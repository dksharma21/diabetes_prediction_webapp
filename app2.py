...
with col2:
    smoking_history = st.selectbox('Smoking History', options=['Never', 'Current', 'Former', 'Ever', 'Not Current', 'No Info'])
    smoking_history_dict = {'Never': 4.0, 'No Info': 0.0, 'Current': 1.0, 'Former': 3.0, 'Ever': 2.0, 'Not Current': 5.0}
    bmi = st.number_input('BMI (0-100)', min_value=0.0, max_value=100.0, step=0.1)
    hba1c_level = st.number_input('HbA1c - Hemoglobin Level (0-9)', min_value=0.0, max_value=9.0, step=0.1)
    blood_glucose_level = st.number_input('Blood Glucose Level (80-200)', min_value=80, max_value=200, step=1)
...
if submit:
    try:
        ...
        st.warning('Please fill all required information. Error: ' + str(e))
